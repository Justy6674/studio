...
export const sendHydrationReminder = functions.https.onCall(async (data: SendReminderInput, context) => {
  ...
  try {
    ...
    if (!toPhoneNumber) {
      ...
      // Not an error, just can't send.
      return { success: false, message: 'User phone number not configured.' };
    }

    if (!/^\+[1-9]\d{1,14}$/.test(toPhoneNumber)) { // Basic E.164 validation
    ...
    if (!messageToSend) {
      // Use a generic message or call generateMotivationalMessage
      // For simplicity here, using a generic message.
      // In a real app, you might call the generateMotivationalMessage function for a personalized touch.
      // const motivationalMessageResult = await generateMotivationalMessage({}, { auth: { uid: targetUserId, token: ''} as any}); // This call needs careful context setup if used
      // messageToSend = (motivationalMessageResult as any).message;
      messageToSend = `Hi ${userName}! Just a friendly reminder to stay hydrated today. Keep up the great work! 💧 - HydroHelper`;
    }
